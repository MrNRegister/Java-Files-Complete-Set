...
public class Start{
	public static void main (String[] args){//This tells Java where the program starts. Static means you can access it without first creating an instance. The static method belongs to the class itself, not an instance of the class.
    ...
    spell.cast(me);//We are telling the spell to cast.
    //CHALLENGE - Can you stop the below from happening if there isn't enough Mana to cast a spell?
    ...
    System.out.println("Here are your selectable targets:");//Display the text listed here.
    	...
    	if(spell instanceof TargetsCreature) {//Check if the spell selected is an instance of TargetsCreature.
    	    ...
    	    String creatureChosen = (String) Utilities.getInput();//Get the input and then call it creatureChosen.
    	    ...
    	    Creature creature = cre.viewCreatures().get(creatureChosenInt);//Go to the Creature class and run the method "viewCreatures" and then select the creature that the user input picked.
    	    System.out.println("You chose: " + creature);//Print out the creature they chose.
	    }
    	else {
    	if(spell instanceof TargetsItem) {//If it isn't; then check if it's an instance of TargetsItem.
    	    ...
    	    Item item = it.viewItems().get(itemChosenInt);//Go to the Item class and run the method "viewItems" and then select the item that the user input picked.
    ...
    System.out.println("You currently have " + me.showMana() + " Mana left.");//system.out says the default output method for this program.
